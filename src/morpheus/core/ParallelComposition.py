...
class ParallelComposition(Composition):
    ...
    def predict(self, X, **kwargs):
        ...
        # Fill in numeric
        t_idx_map_numeric = self._map_elements_idx(
            self.numeric_targ_ids, self.targ_ids, return_array=True
        )
        t_idx_numeric, t_idx_s_numeric = (
            t_idx_map_numeric[:, 0],
            t_idx_map_numeric[:, 1],
        )
        ...
        for t_idx_nominal, t_idx_s_nominal in t_idx_map_nominal:
            s_pred[:, t_idx_s_nominal] = self.classes_[t_idx_nominal].take(
                np.argmax(s_nominal_proba[t_idx_nominal], axis=1), axis=0
            )
    ...
    def predict_nominal(self, X, **kwargs):
        ...
        for e in relevant_estimators:
            # Filter desc. atts
            d_idx_map = self._map_elements_idx(
                e.desc_ids, self.desc_ids, return_array=True
            )
            d_idx_comp = d_idx_map[:, 1]

            # Do algo
            e_nominal = self._predict_nominal_estimator_tidy(
                e, X[:, d_idx_comp], **kwargs
            )
